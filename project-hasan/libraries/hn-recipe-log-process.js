...
function submit_form(){
    ...
    _("php_part").style.display = "block";
    xhttp.onreadystatechange=function() {
        if (this.readyState == 4 && this.status == 200) {
            _("php_emit").innerHTML = this.responseText;
        }
        
    };
    
    xhttp.open("GET","libraries/hn_process_receipt.php?name="+name+"&difficulty="+difficulty+"&ingredients_list="+ingredients_list+"&type_of_drink="+type_of_drink+"&process="+process, true);
    xhttp.send();

    _("submit-button").style.display = "none";
    _("home-button").style.display = "block";
